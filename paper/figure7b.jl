...
function fig_gen()
    ...
    SL, DL = single_double_layer(pde,X,Γ;correction=:nothing)
    U  = DL*σ - im*k*SL*σ
    ...
    fig    = heatmap(x,y,field_error,
                     ...
                     size=(450,400))
    plot!(fig,Γ,lc=:black)
    @show length(Γ), norm(γ₁u - (H*σ - im*k*(ADL*σ - σ/2)),Inf), maximum(field_error)
    return fig
end

fig = fig_gen()
fname = "/home/lfaria/Dropbox/Luiz-Carlos/general_regularization/draft/figures/fig7b.pdf"
savefig(fig,fname)
display(fig)
