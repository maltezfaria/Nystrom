...
function fig_gen(p,h,k)
    ...
    gmsh.option.setNumber("Mesh.ElementOrder", p+1)
    ...
    @show norm(Ua-Ue,Inf)
    sol_real = vec([ [real(Ua[n] + Ui[n])] for n in 1:length(Ue)])
    ee       = vec([ [abs(Ua[n] - Ue[n])] for n in 1:length(Ue)])
    # sol_real = vec([ [real(Ua[n] - Ue[n])] for n in 1:length(Ue)])
    ...
    #write
    gmsh.view.write(view_solution, "/home/lfaria/Dropbox/Luiz-Carlos/general_regularization/draft/figures/sphere_solution_nocorrection.msh")
    gmsh.view.write(view_error, "/home/lfaria/Dropbox/Luiz-Carlos/general_regularization/draft/figures/sphere_error_nocorrection.msh")
    gmsh.finalize()
    return nothing
end
