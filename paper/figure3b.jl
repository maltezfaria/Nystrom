...
function convergence_exterior_dirichlet3d()
    ...
    for (p,h) in zip(qorder,el_size)
        ...
        for n in 1:1:niter[cc]
            ...
            σ,ch      = gmres(L,γ₀u,verbose=true,log=true,tol=1e-14,restart=100,maxiter=100)
            ...
            push!(eFar,norm(D*σ - η*(S*σ) - Ufar,Inf))
        ...
        plot!(fig,dof_per_dim,eFar,label="Farfield error for p=$p",m=:x,color=cc)
