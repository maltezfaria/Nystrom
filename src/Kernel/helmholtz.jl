...
default_density_eltype(::Helmholtz{N}) where {N} = ComplexF64
...
default_kernel_eltype(::HelmholtzPML)  = ComplexF64
default_density_eltype(::HelmholtzPML) = ComplexF64
...
# Adjoint double Layer Kernel
function (ADL::AdjointDoubleLayerKernel{T,S})(x,y,nx)::T where {T,S<:HelmholtzPML}
    τ  = ADL.op.τ
    A  = ADL.op.A(x)
    x  = τ(x)
    y  = τ(y)
...
# Hypersingular kernel
function (HS::HyperSingularKernel{T,S})(x,y,nx,ny)::T where {T,S<:HelmholtzPML}
    τ  = HS.op.τ
    Ax  = HS.op.A(x)
    Ay  = HS.op.A(y)
    x  = τ(x)
